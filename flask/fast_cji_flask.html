...
<html lang="en-Us">
    <head>
        <title>Fast CGI Flask - BestPythonTutorials.com</title>
        <meta charset="utf-8">
        ...
        <script src="../codemirror/mode/python/python.js"></script>
         <script src="../codemirror/mode/python/python.js"></script>
          ...
          <script src="../codemirror/mode/sql/sql.js"></script>
           ...
           <meta name="description" content="Fast CGI Flask - BestPythonTutorials.com" />
...
<a class="btnxc" onclick=""><i class="fa fa-chevron-up"></i></a>
 ...
 <ul class="menu text-center">
    ...
    <li style="box-shadow:1px 1px white;background-color:white;background-image:none!important;padding:0px;"><h4 style="text-align:center;font-family: 'Big Shoulders Stencil Text', cursive!important;height:30px;line-height:55px;color:#a54eee;background-color:white;font-size:35px!important;">FLASK TUTORIAL</h4></li><br>
      ...
      <li style="background-image:linear-gradient(#4537d1,#4537d1);border-top:1px solid white;border-bottom:1px solid white;"><a href="fast_cji_flask.php"  target="_self">Fast CGI</a></li>
...
<div id="myNav" class="overlay">
  ...
  <div class="overlay-content">
     ...
     <a href="" style="border:none;color:white;"><h1 id="tag1">FLASK TUTORIAL</h1></a>
      ...
      <a href="fast_cji_flask.php"  target="_self" style="color:white;">Fast CGI</a>
...
<!--search ends-->
    <h2 style="text-align:left;color:#a54eee;">Fast CGI - Flask</h2>
<div class="row ml-auto mr-auto" style="width:40%!important;">
<a href="https://www.facebook.com/sharer/sharer.php?u=http%3A//bestpythontutorials.com/flask/fast_cji_flask.php" style="background-color:#3377ff;color:white;border-radius:1px;padding:1px;font-size:15px;" id="facebookshare" class="col-lg-3 col-sm-3 col-md-3 mb-1" class="col-lg-3 col-sm-3 col-md-3 mb-1"> <i class="fa fa-facebook-f"></i> Share </a>
<a href="https://twitter.com/intent/tweet?text=http%3A//bestpythontutorials.com/flask/fast_cji_flask.php" style="background-color:#1ac6ff;color:white;border-radius:1px;padding:1px;font-size:15px;" id="facebookshare" class="col-lg-3 col-sm-3 col-md-3 mb-1"> <i class="fa fa-twitter"></i> Tweet </a>
...
<!--content goes here-->
<div id="main">
<p style="text-align:justify;" class="mt-4">FastCGI is a deploying option for Flask application on web servers like nginix, lighttpd, and Cherokee.To use your WSGI application with any of them you will need a FastCGI server first. The most popular one is flup which we will use for this guide. Make sure to have it installed to follow along.</p>

<h3 class="text-left mt-4" style="color:#a54eee;">Configurations</h3>
<p style="text-align:justify;">Firstly we have to configure the FastCGi.In order to do this you have create a FastCGI file with <b>.fcgi</b> extension.In our case we are calling it <b>fast.fcgi</b>.Now what is the content of this file check below:</p>

<div class="text-left mt-0 mb-1">
  <p class="text-left" class="mb-0 mt-1">//fast.fcgi</p>
...
<p style="text-align:justify;" class="mt-4">This is enough for Apache to work, however nginx and older versions of lighttpd need a socket to be explicitly passed to communicate with the FastCGI server. For that to work you need to pass the path to the socket to the WSGIServer:</p>

<div class="text-left mt-0 mb-1">
  <p class="text-left" class="mb-0 mt-1"></p>
 <form action="" method="post" target="_blank">
 <textarea id="editor2" name="initScript" style="height:20vh!important;text-align:justify;overflow:hidden;-webkit-border-radius: 5px;
    -moz-border-radius: 5px;border-radius: 5px;z-index:0;" readOnly = 'nocursor'>WSGIServer(application, bindAddress = '/path/to/fcgi.sock').run()        </textarea>
              <!--<button type="submit" style="background-color:white;border:none;outline:none;box-shadow:none;" class="mt-3">
              <a type="submit" class="pl-4 pr-4" style="border:2px solid #3399ff;border-radius:4px;color:#1aa3ff;">Run it    <i class="fa fa-angle-right"></i><i class="fa fa-angle-right"></i></a>
             </button>
             -->
...
<h3 class="text-left mt-4" style="color:#a54eee;">Configuring Apache</h3>
<p style="text-align:justify;">In this section we are going to learn how to configure Apache server for our Application.The example above is good enough for a basic Apache deployment but your .fcgi file will appear in your application URL. There are few ways to configure your application so that yourapplication.fcgi does not appear in the URL. A preferable way is to use the ScriptAlias and SetHandler configuration directives to route requests to the FastCGI server. The following example uses FastCgiServer to start 5 instances of the application which will handle all incoming requests:</p>
...
<h3 class="text-left mt-4" style="color:#a54eee;">Configuring lighttpd</h3>
<p style="text-align:justify;">Now we will try to configure the lighttpd.Basic configuration of lighttpd looks like this - </p>
<div class="text-left mt-0 mb-1">
  <p class="text-left" class="mb-0 mt-1"></p>
 <form action="" method="post" target="_blank">
...
url.rewrite-once = (
    "^(/static($|/.*))$" => "$1",
    "^(/.*)$" => "/yourapplication.fcgi$1"   </textarea>
              <!--<button type="submit" style="background-color:white;border:none;outline:none;box-shadow:none;" class="mt-3">
              <a type="submit" class="pl-4 pr-4" style="border:2px solid #3399ff;border-radius:4px;color:#1aa3ff;">Run it    <i class="fa fa-angle-right"></i><i class="fa fa-angle-right"></i></a>
             </button>
             -->
...
<h3 class="text-left mt-4" style="color:#a54eee;">Configuring nginx</h3>
<p style="text-align:justify;">Installing FastCGI applications on nginx is a bit different because by default no FastCGI parameters are forwarded.
</p>
...
function next(){
  window.open("deployment_flask.php","_self");
}
function prev(){
  window.open("wtf_flask.php","_self");
}
