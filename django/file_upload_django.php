...
<html lang="en-Us">
    <head> 
        <title>File Upload - Django</title>
        <meta charset="utf-8">
...
<script async src="https://cse.google.com/cse.js?cx=016236012273495808262:cabujlbzuwi"></script>
        ...
        <meta name="description" content="File Upload in Django - Uploading Files in Django">
...
<a class="btnxc" onclick=""><i class="fa fa-chevron-up"></i></a>
  ...
  <!--vertical menu-->
    ...
    <ul class="menu text-center">
      ...
      <li style="background-image:linear-gradient(to right,#4537d1,#4537d1);color:white;border-bottom:1px solid white;border-top:1px solid white;text-align:left;"><a href="file_upload_django.php"  target="_self">File Upload</a></li>
...
<div id="myNav" class="overlay">
  ...
  <div class="overlay-content">
   ...
   <a href="home_django.php"  target="_self">Django Home</a>
      ...
      <a href="file_upload_django.php"  target="_self" style="color:white;">File Upload</a>
...
<h2 class="text-left" style="color:#a54eee;">File Upload - Django</h2>

<div class="row ml-auto mr-auto" style="width:40%!important;">
<a href="https://www.facebook.com/sharer/sharer.php?u=http%3A//bestpythontutorials.com/django/file_upload_django.php" style="background-color:#3377ff;color:white;border-radius:1px;padding:1px;font-size:15px;" id="facebookshare" class="col-lg-3 col-sm-3 col-md-3 mb-1" class="mr-2 pl-2 pr-2"> <i class="fa fa-facebook-f"></i> Share </a>
<a href="https://twitter.com/intent/tweet?text=http%3A//bestpythontutorials.com/django/file_upload_django.php" style="background-color:#1ac6ff;color:white;border-radius:1px;padding:1px;font-size:15px;" id="facebookshare" class="col-lg-3 col-sm-3 col-md-3 mb-1"> <i class="fa fa-twitter"></i> Tweet </a>
...
<p style="text-align:justify;" class="mt-4">In this chapter we are going to see the implementation of File Upload System in Django.It's very easy to implement it.</p>
<p style="text-align:justify;">When Django handles a file upload, the file data ends up placed in <b>request.FILES</b> (for more on the request object see the documentation for <a href="response_request_django.php" style="color:grey;text-decoration:none;">request and response objects <img src="link.png" class="img-fluid" style="height:10px;width:10px;"></a>). This document explains how files are stored on disk and in memory, and how to customize the default behavior</p>
...
<!------------------------------ADs------------------------------>
<div class="container mb-3 mt-3 text-center" style="height:20vh;width:100%;">
...
<p style="text-align:justify;">The <b>forms.FileField()</b> method is used to create a file input and submit the file to the server. While working with files, make sure the HTML form tag contains <b>enctype="multipart/form-data"</b> property.</p>

<h3 class="text-left mt-4">Basic File Upload System</h3>
<p style="text-align:justify;" class="mb-1">Setting up the template(index.html).</p>
...
<body>
  <form method="POST" class="post-form" enctype="multipart/form-data">  
        {% csrf_token %}  
        {{ form.as_p }}  
        <button type="submit" class="btn newcolorbtn">Submit</button>  
</form> 
</body>
</html></textarea>
...
<p style="text-align:justify;" class="mt-3">Main form code as <b>forms.py</b>(You have to create this file of your own).</p>
<div class="text-left mt-0 mb-1">
...
class InfoForm(forms.Form):   
    file = forms.FileField() # for creating the file input </textarea>
              <!--<button type="submit" style="background-color:white;border:none;outline:none;box-shadow:none;" class="mt-3">
              <a type="submit" class="pl-4 pr-4" style="border:2px solid #3399ff;border-radius:4px;color:#1aa3ff;">Run it    <i class="fa fa-angle-right"></i><i class="fa fa-angle-right"></i></a>
             </button>
             -->
...
<p style="text-align:justify;" class="mt-3">Making changes on <b>views.py</b> and <b>urls.py</b> file</p>
<div class="text-left mt-0 mb-1">
...
from yourapp.functions.functions import upload_file  
from yourapp.form import InfoForm  
def index(request):  
    if request.method == 'POST':  
        data = InfoForm(request.POST, request.FILES)  
        if data.is_valid():  
            upload_file(request.FILES['file'])  
            return HttpResponse("File is uploaded successfuly")  
    else:  
        data = Form()  
        return render(request,"index.html",{'form':info})   </textarea>
              <!--<button type="submit" style="background-color:white;border:none;outline:none;box-shadow:none;" class="mt-3">
              <a type="submit" class="pl-4 pr-4" style="border:2px solid #3399ff;border-radius:4px;color:#1aa3ff;">Run it    <i class="fa fa-angle-right"></i><i class="fa fa-angle-right"></i></a>
             </button>
             -->
...
<p style="text-align:justify;" class="mt-3">Now the last step is to make changes on <b>urls.py</b> file</p>
<div class="text-left mt-0 mb-1">
...
<p style="text-align:justify;" class="mt-3">This is the main file which is used to gain the upload file functionality.</p>
<div class="text-left mt-0 mb-1">
  <p style="text-align:left;" class="mt-0 mb-0 text-center">functions.py</p>
  <form action="https://www.jdoodle.com/api/redirect-to-post/python3-programming-online" method="post" target="_blank">
 <textarea id="editor5" name="initScript" style="height:20vh!important;text-align:justify;overflow:hidden;-webkit-border-radius: 5px;
    -moz-border-radius: 5px;border-radius: 5px;z-index:0;" readOnly = 'nocursor'>
def upload_file(f):  
    with open('yourapp/static/upload/'+ f.name, 'wb+') as destination:  
        for chunk in f.chunks():  
            destination.write(chunk)  </textarea>
              <!--<button type="submit" style="background-color:white;border:none;outline:none;box-shadow:none;" class="mt-3">
              <a type="submit" class="pl-4 pr-4" style="border:2px solid #3399ff;border-radius:4px;color:#1aa3ff;">Run it    <i class="fa fa-angle-right"></i><i class="fa fa-angle-right"></i></a>
             </button>
             -->
...
<p style="text-align:justify;">Please remember that you have to create a folder in the static folder with the name upload if you follow our examples and put the files which you want for uploading.</p>
...
function prev(){
  window.open("generic_views_django.php","_self");
}
